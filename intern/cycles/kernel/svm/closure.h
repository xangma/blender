                          ...
                          int offset)
{
  ...
  switch (type) {
    case CLOSURE_BSDF_PRINCIPLED_ID: {
      ...
      N = svm_ocean_shading_normal(kg, sd, stack_valid(data.normal_offset), N);
    ...
    case CLOSURE_BSDF_DIFFUSE_ID: {
      ...
      N = safe_normalize_fallback(N, sd->N);
      N = svm_ocean_shading_normal(kg, sd, stack_valid(bsdf_data.normal_offset), N);
    ...
    case CLOSURE_BSDF_F82_CONDUCTOR: {
      ...
      if (bsdf != nullptr) {
        ...
        N = safe_normalize_fallback(N, sd->N);
        N = svm_ocean_shading_normal(kg, sd, stack_valid(cdata.normal_offset), N);
        const float3 valid_reflection_N = maybe_ensure_valid_specular_reflection(sd, N);
