...
#  define OCEAN_CO(_size_co_inv, _v) ((_v * _size_co_inv) + 0.5f)
  ...
  blender::bke::SpanAttributeWriter<float3> ref_coord_attr;
  blender::bke::SpanAttributeWriter<float3> geometry_normal_attr;
  blender::bke::SpanAttributeWriter<float3> geometry_support_cov_attr;
  blender::bke::SpanAttributeWriter<float2> ref_uv_attr;
  blender::bke::SpanAttributeWriter<float> camera_lod_level_attr;
  blender::bke::SpanAttributeWriter<float> camera_lod_split_level_attr;
  blender::bke::SpanAttributeWriter<float> camera_lod_morph_attr;
  blender::bke::SpanAttributeWriter<float> camera_lod_radius_attr;
...
static void panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  if (generate_mode) {
    ...
    if (use_camera_lod) {
      uiLayout *warning = &col->column(false);
      warning->label(RPT_("Camera LOD is Cycles-only for final renders; unsupported paths use dense geometry"),
                     ICON_INFO);
      if (stereo_dataset_mode) {
        warning->label(
