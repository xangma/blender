                                              ...
                                              const OceanCameraLODMeshCacheKey &b)
{
  return a.frame == b.frame && a.resolution == b.resolution &&
         ...
         a.size == b.size && a.foam_fade == b.foam_fade &&
         a.lod_pixel_error == b.lod_pixel_error &&
...
#  define OCEAN_CO(_size_co_inv, _v) ((_v * _size_co_inv) + 0.5f)
  ...
  if (use_camera_lod) {
    ...
    bke::MutableAttributeAccessor attributes = result->attributes_for_write();
    ref_coord_attr = attributes.lookup_or_add_for_write_only_span<float3>(
        OCEAN_ATTR_REF_COORD, bke::AttrDomain::Point);
